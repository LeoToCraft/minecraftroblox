...
Текущая версия проекта: `0.0.9a`.

Использовать два режима изменения версии:

1. Баг-фиксы, маленькие технические правки, внутренние исправления и похожие изменения увеличивают дополнительный счетчик после основной версии.
   - `0.0.9a` -> `0.0.9.1a`
   - `0.0.9.1a` -> `0.0.9.2a`
   - `0.0.9.3a` -> `0.0.9.4a`
2. Крупные, средние и обычные маленькие обновления увеличивают основную версию на 1 и сбрасывают счетчик технических фиксов.
   - `0.0.8a` -> `0.0.9a`
   - `0.0.9a` -> `0.0.10a`
   - `0.0.9.7a` -> `0.0.10a`

Суффикс `a` сохранять, пока пользователь явно не попросит перейти на другой канал/обозначение версии.

Если человек или агент исправил баг, сделал техническую правку или добавил обновление, он на свое усмотрение должен переименовать версию в подходящую по этим правилам. Версию нужно менять в местах, где она отображается или передается в игре, чтобы клиент и сервер не расходились.
